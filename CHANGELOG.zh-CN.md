...
- 桌面端恢复终端时只挂载当前焦点那一格，丢掉异常膨胀的分屏布局，并且不再把主机上每一个存活会话都加成新的分屏。

## [2.0.30] - 2026-08-28

### 新增

- 桌面端 CNB 侧栏可直接评论、关闭或重新打开 issue/PR，以及停止运行中的流水线，底层仍走 `codux-cnb`。
...
- 修复移动端终端切换和重连时旧选区、修饰键状态残留的问题；离开工作区时释放远程 viewport，删除当前设备时完整清理旧运行态。
- 修复终端历史消息与 envelope 序列处理，避免晚到的历史基线被误判为过期实时输出而丢弃。
- 文件编辑器关闭前会确认未保存修改，保存成功后会更新干净基线。

## [2.0.27] - 2026-08-19

### 修复

- 修复移动端终端恢复：保留选中的项目和工作区、在中继重连后补齐更多历史输出、避免终端工具遮挡输入框，并在终端缺失时正确创建而不丢失项目选择。
- AI 会话列表会在异步索引更新和重连后刷新；AI 用量统计支持按需刷新，并限定在当前选中的远程项目内。

## [2.0.26] - 2026-08-18

### 修复
- 手机端切换到没有终端的项目时保持项目/终端切换页打开，不再展示空白终端面板；终端可用后会自动进入终端页。

## [2.0.25] - 2026-08-17

### 修复
- 手机端打开文件时立即弹出编辑器并显示加载中，不再被文件列表路由挡住。
- 桌面端与 agent 的 AI 用量统计改为跟随当前 worktree，与会话列表一致（无 worktree 的项目仍按项目汇总）。

## [2.0.24] - 2026-08-17

### 修复
- 修复切换到尚无终端的项目后，主机下一次下发项目列表会把选中项目弹回原项目的问题。

## [2.0.23] - 2026-08-17

### 修复
- 修复手机端切换项目后 UI 不刷新的问题（runtime 状态更新后未触发重建）。
- 手机切换页选中其他项目后立即返回终端视图。

## [2.0.22] - 2026-08-17

### 修复
- 更新 widget 测试：合并切换页已打开时不再重复尝试打开菜单。

## [2.0.21] - 2026-08-17

### 修复
- 移除未使用的 import，修复 mobile release analyze 步骤失败。

## [2.0.20] - 2026-08-17

### 变更
- 合并手机端项目切换与终端切换：切换页顶部显示项目条，下方为终端/Worktree/会话列表。
- 手机溢出菜单统一为「切换项目/终端」单一入口。

## [2.0.19] - 2026-08-17

### 修复
- 修正 mobile 断线 widget 测试：断言 zh-CN 重连文案，并在断言期间 stall fake transport 的自动重连。

## [2.0.18] - 2026-08-17

### 修复
- 在 mobile 断线 widget 测试中 stall fake transport 的重连，避免断言前自动重连已完成。
...
### 修复
- 更新 mobile widget 测试，匹配终端页内重连流程与断线工具页提示行为。

## [2.0.15] - 2026-08-17

### 修复
- 修复终端悬浮工具球样式补丁中残留的 mobile 编译失败，将装饰常量提升到文件作用域。

## [2.0.14] - 2026-08-17

### 修复
- 修复终端悬浮工具球样式改动导致的 mobile release `flutter analyze` 失败。
...
### 新增
- 窄屏手机工作区改为单行顶栏 + 溢出菜单；AI 统计、文件和 Git 以全屏页面打开，终端保持在底层挂载。
- 窄屏手机支持 Git 状态与 diff 浏览，并可通过溢出菜单切换项目。

### 修复
- 修复 Windows / Linux 桌面端构建，并恢复 packaging 测试 workflow。

## [2.0.11] - 2026-08-16

### 修复
- 修复 Windows / Linux 上 Kimi Code 等 CLI 的 Ctrl+S「插队/中断」快捷键无效的问题：终端聚焦时将快捷键转发给 PTY，而不是触发保存文件。
- 修复 macOS 开发者性能监控 HUD 反复执行 `ps` 采样 CPU、可能触发公司 EDR/安全策略的问题。
...
- 修复 Linux Agent 在 worktree 下 AI 用量统计不准确的问题：按 worktree 作用域读取实时 session，索引完成后自动向手机推送更新，并统一 `ai.session` 的 worktree 路径解析。
- 修复手机连纯 Agent 时 viewport 过期后误提示「桌面端接管」的文案。

## [2.0.7] - 2026-08-16

### 修复

- headless Agent 现已支持手机端终端文件上传，行为与桌面端主机一致，并正确回复 `terminal.uploaded`。

## [2.0.4] - 2026-08-14

### 新增

- 为托管远程 Agent runtime 增加 SSH 与工具权限处理。

### 调整

- 重做设置页、工作区侧栏和项目徽章。未自定义图标时，徽章使用项目名的第一个字符。
- 桌面更新器改用本仓库 GitHub 发版流水线的个人签名公钥。

### 修复

- Linux 上启用 `notify-rust` 的 zbus 支持，使系统通知可以正常编译。
