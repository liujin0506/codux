...
describe("terminal text input adapter", () => {
  ...
  it("does not write composing text but accepts committed non-ascii text events", async () => {
    const textarea = new EventTarget();
    const write = vi.fn();
    const adapter = installTerminalTextInputAdapter({
      textarea,
    ...
    dispatchTextInput(textarea, "你", { isComposing: true });
  ...
  it("writes committed IME punctuation from beforeinput without a key map", async () => {
    const textarea = new EventTarget();
    const write = vi.fn();
    const adapter = installTerminalTextInputAdapter({
      textarea,
  ...
  it("does not duplicate text that xterm emitted before the input event", async () => {
    const textarea = new EventTarget();
    const write = vi.fn();
    const adapter = installTerminalTextInputAdapter({
      textarea,
